...
partial class Rogue
{
        ...
        switch (obj.o_which)
        {
            case WS_LIGHT:
                ...
                else
                {
                    ...
                    if (!terse)
                        addmsg(" by a shimmering {0} light", pick_color("blue"));
                    endmsg();
                }
                break;
            case WS_DRAIN:
    ...
    void drain()
    {
        ...
        if (chat(hero.y, hero.x) == DOOR)
            ...
            corp = null;
        inpass = (bool)((proom.r_flags & ISGONE) != 0);
    ...
    void fire_bolt(coord start, coord dir, string name)
    {
                    ...
                    if (!hit_hero && (tp = moat(pos.y, pos.x)) != null)
                    {
                        ...
                        else if (ch != 'M' || tp.t_disguise == 'M')
                        {
                            ...
                            if (terse)
                                ...
                                msg("the {0} whizzes past {1}", name, set_mname(tp));
                        }
                    }
        ...
        for (c2 = 0; c2 < c1; c2++)
            mvaddch(spotpos[c2].y, spotpos[c2].x, chat(spotpos[c2].y, spotpos[c2].x));
    }
